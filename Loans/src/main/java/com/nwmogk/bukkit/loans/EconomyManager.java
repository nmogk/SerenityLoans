 ...
 * ========================================================================
 * 
 * File: EconomyManager.java Contributing Authors: Nathan W Mogk
 * 
 * This class accepts all economy-related calls from other classes in the SerenityLoans plugin and
 * performs the operation on the proper economy implementation, which is defined in the
 * configuration. It provides most of the functionality of a Vault economy with additional features,
 * such as currency symbols and percentage formatting.
 * 
 * 
 * ======================================================================== 
 * LICENSE INFORMATION
...
public class EconomyManager {
	...
	private enum EconType {
		VAULT, HYBRID, SERENE, INTERNAL
	};
	...
	public EconomyResponse convertEconResult( EconResult result ) {
		...
		if ( config != EconType.VAULT && config != EconType.HYBRID ) { return null; }

		return new EconomyResponse( result.amount, result.balance,
				result.callSuccess ? EconomyResponse.ResponseType.SUCCESS
						: EconomyResponse.ResponseType.FAILURE, result.errMsg );
	}
	...
	public EconResult deposit( FinancialEntity entity, double amount ) {
		...
		if ( entity == null ) return new EconResult( 0, 0, false, "Entity is not recognized." );

		if ( amount < 0 ) return new EconResult( 0, 0, false, "Amount query is negative!" );

		if ( entity.getPlayerType().equals( PlayerType.CREDIT_UNION )
				&& config.equals( EconType.VAULT ) )
			return new EconResult( 0, 0, false, "CreditUnions are not supported by the economy." );

		if ( config.equals( EconType.VAULT )
				|| ( config.equals( EconType.HYBRID ) && entity.getPlayerType().equals(
						PlayerType.PLAYER ) ) )
			return new EconResult( econ.depositPlayer(
					plugin.playerManager.getOfflinePlayer( entity.getUserID() ), amount ) );
	...
	@Deprecated
	public EconResult deposit( String name, double amount ) throws InterruptedException,
	...
	public String format( double amount ) {
		...
		if ( !useSym ) dollars += amount == 1 ? currencyNameSingular() : currencyNamePlural();
	...
	@Deprecated
	public EconResult getBalance( String name ) throws InterruptedException, ExecutionException,
			TimeoutException {
	...
	public EconResult getBalance( FinancialEntity entity ) {
		...
		if ( entity.getPlayerType().equals( PlayerType.CREDIT_UNION ) ) {
			...
			if ( config.equals( EconType.VAULT ) || config.equals( EconType.HYBRID ) )
				balance = econ.getBalance( plugin.playerManager.getOfflinePlayer( entity
						.getUserID() ) );
			else
				balance = entity.getCash();
		}
	...
	/**
	 * Returns an EconResult which contains the balance of the entity and the success of the amount
	 * query. Depends on the implementation of getBalance(FinancialEntity) to get the balance.
	 * 
	 * @param entity
	 * @param amount
	 * @return
	...
	@Deprecated
	public EconResult has( String name, double amount ) throws InterruptedException,
			ExecutionException, TimeoutException {
	...
	@Deprecated
	public boolean hasBalance( String financialEntity, double amount ) throws InterruptedException,
			ExecutionException, TimeoutException {
	...
	/**
	 * This method attempts to withdraw cash from a financial entity's account. It will handle all
	 * of the various economy options available. Returns a EconResult, which has similar
	 * functionality to the Vault EconomyResponse.
	 * 
	 * @param entity
	 * @param amount
	 * @return
	...
	public EconResult withdraw( FinancialEntity entity, double amount ) {
		...
		if ( config.equals( EconType.VAULT )
				|| ( config.equals( EconType.HYBRID ) && entity.getPlayerType().equals(
						PlayerType.PLAYER ) ) )
			return new EconResult( econ.withdrawPlayer(
					plugin.playerManager.getOfflinePlayer( entity.getUserID() ), amount ) );
	...
	@Deprecated
	public EconResult withdraw( String name, double amount ) throws InterruptedException,
			ExecutionException, TimeoutException {
	...
	public boolean hasBankSupport() {
		...
		if ( config == EconType.VAULT ) return econ.hasBankSupport();
	...
	public List<String> getBanks() {
		...
		// TODO evaluate eliminating this for conversion purposes
		if ( config == EconType.VAULT ) return econ.getBanks();
	...
	public boolean hasAccount( OfflinePlayer player ) {
		...
		// TODO evaluate eliminating this for conversion purposes
		if ( config == EconType.VAULT ) return econ.hasAccount( player );
	...
	public EconResult createBank( String bankName, OfflinePlayer manager ) {
		...
		return new EconResult( 0, 0, plugin.playerManager.createFinancialInstitution( bankName,
				entity ), null );
	}
}
